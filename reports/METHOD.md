# Liga Five Guys — how the numbers are made — 2026-09-15 05:46 UTC
...
|---|---|
| Jornada 5 is half played — 18 clubs are done | their points are already in `now`, so only the rest of the round is simulated, and it still re-picks an eleven that is in fact already locked |
...
| Shape prior | shape from 1553 observed matches |
| P(start) fit | P(start) fitted on 653 confirmed starts across 20 team sheets: futbolfantasy recalibrated (logit +0.0 +1.0x), blended 10% with analiticafantasy where it has an opinion (a named starter counts 72%). Brier improves 0.000 on line-ups the fit had not seen |
...
|---|---|---|---|--:|---|---|
| 🟢 | api_activity | every transfer, which is what the ledger replays — one row per deal, so the newest is the last deal and not the last sweep | LaLiga Fantasy API | 200 | 15 Sep 05:43 | fetched 3 minutes ago |
| 🟢 | api_leagues | your cash and the league's id | LaLiga Fantasy API | 189 | 15 Sep 05:43 | fetched 3 minutes ago |
| 🟢 | api_lineup | the eleven you have actually fielded, and the formation the app says you are playing | LaLiga Fantasy API | 1,826 | 15 Sep 05:43 | fetched 3 minutes ago |
| 🟢 | api_market | what is on offer, and the bids on it | LaLiga Fantasy API | 5,776 | 15 Sep 05:43 | fetched 3 minutes ago |
| 🟢 | api_players | names for players nobody owns any more — one row per player, first sighting kept | LaLiga Fantasy API | 128 | 15 Sep 05:43 | fetched 3 minutes ago |
| 🟢 | api_standings | the league table — position, points, squad value, and your balance | LaLiga Fantasy API | 945 | 15 Sep 05:43 | fetched 3 minutes ago |
| 🟢 | api_stats | what the app scored each player, broken into what he did — one row per player per week per stat, a correction being a later row rather than an overwrite | LaLiga Fantasy API | 10,411 | 15 Sep 05:43 | fetched 3 minutes ago |
| 🟢 | api_teams | all five squads | LaLiga Fantasy API | 13,466 | 15 Sep 05:43 | fetched 3 minutes ago |
| ⚪ | clubs | the same, for clubs | src/crosswalk.py | 20 | — | rebuilt every run from the tables above |
| 🟢 | elo | team strength, which ranks the fixture term | clubelo.com | 3,420 | 15 Sep 05:43 | fetched 3 minutes ago |
| 🟢 | fixtures | who plays whom next, for the fixture term | analiticafantasy.com | 2,466 | 15 Sep 05:43 | fetched 3 minutes ago |
| 🟢 | lineups | probable XI percentages, both sources | analiticafantasy.com, futbolfantasy.com ×40 | 146,883 | 15 Sep 05:43 | fetched 3 minutes ago |
| 🟢 | market | price, value, position, fitness — every player in the game | futbolfantasy.com | 147,687 | 15 Sep 05:43 | fetched 3 minutes ago |
| 🟢 | matches | fixtures, kickoffs, results | futbolfantasy.com | 72,960 | 15 Sep 05:43 | fetched 3 minutes ago |
...
| 🟢 | points | realised points per jornada, the actuals in every table below | futbolfantasy.com | 1,604 | 15 Sep 05:43 | fetched 3 minutes ago |
| 🟢 | starters | confirmed elevens, which is what P(start) is graded on | futbolfantasy.com | 166,048 | 15 Sep 05:43 | fetched 3 minutes ago |
...
| Home advantage | +13.2% | yes — fit from 1244 real matches (home 1.52, away 1.16 goals/match, ratio 1.304) |
...
| Season spread, match to match | each round resamples a real per-match score, rescaled to the player's rate | from 1553 observed matches |
...
| Measure | Value |
|---|--:|
| Player-intervals scored (2026-27) | 57 |
| Predicted, total | 202 pts |
| Actual, total | 251 pts |
| **Mean absolute error (per match played)** | **2.9 pts** |
| Pairs predating the fixture term | 8 of 57 |
...
_25 of 57 intervals overpredicted, 32 underpredicted (mean signed error -0.9 pts) — the "Biggest miss" table below is the tail, not the whole picture._

_vs. a trivial guess (everyone scores the sample's own mean, 4.4 pts/match, no player identity at all): ours 2.85 MAE, that guess 3.00 MAE — does not clearly beat it yet (90% CI on the gap: -0.89 to +0.60 pts, straddles zero)._
...
|---|--:|--:|--:|--:|
| harder | 27 | 3.6 | 5.1 | -1.6 |
| neutral | 11 | 4.4 | 4.7 | -0.4 |
| easier | 11 | 3.9 | 3.6 | +0.3 |
...
**Fit from real data this run: 1.00** (h3 wasn't more variable than h1 (0.179 vs 0.191, rate_rel-normalised) — no evidence DRIFT_FRAC should move from 1.00).

### Who to believe about the eleven

| Source | Calls | Mean claim | Hit | Brier |
|---|--:|--:|--:|--:|
| **starts** — 1078 confirmed, 48 locked round(s) | | | | |
| analitica | 842 | 81% | 67% | 0.200 |
| futbolfantasy ←read | 2512 | 35% | 35% | 0.069 |
| our forecast | 77 | 68% | 77% | 0.086 |
| analitica — named, no number | 103 | — | 51% | — |

| **starts, same population AND same instances as our forecast only** — the real fair comparison (n=77 claims) | | | | |
| analitica | 48 | 93% | 81% | 0.139 |
| futbolfantasy ←read | 74 | 69% | 78% | 0.077 |
| our forecast | 77 | 68% | 77% | 0.086 |
_vs analitica: 90% CI on the Brier gap -0.145 to +0.013 — no significant difference._
_vs futbolfantasy: 90% CI on the Brier gap -0.031 to +0.036 — no significant difference._
...
| analitica | 2739 | 80% | 19% | 0.588 |
| futbolfantasy ←read | 8523 | 35% | 10% | 0.221 |
| our forecast | 348 | 61% | 18% | 0.383 |
| analitica — named, no number | 581 | — | 9% | — |
| futbolfantasy — named, no number | 4 | — | 0% | — |

| Not graded | Calls |
|---|--:|
| within 10 points of 50%, on starts | 537 |
| jornada 1 — its opener kicked off before this repo saw a kickoff for it, so there is no honest cutoff | all |
| within 10 points of 50%, on appearances | 1696 |
...
_Our forecast vs. trivial baselines, n=82 — a DIFFERENT sample than the tables above (this one joins the rate and start sides on jornada via golden_rows(), no 10-point undecided-band exclusion): ours 0.096, a flat 50% guess 0.250, a constant 67% guess 0.196 (Brier, lower is better) — beats both, adding real information._
