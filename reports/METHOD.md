# Liga Five Guys — how the numbers are made — 2026-09-06 20:11 UTC
...
|---|---|
| Jornada 4 is half played — 8 clubs are done | their points are already in `now`, so only the rest of the round is simulated, and it still re-picks an eleven that is in fact already locked |
...
| P(start) fit | P(start) fitted on 633 confirmed starts across 20 team sheets: futbolfantasy recalibrated (logit +0.0 +1.0x), blended 30% with analiticafantasy where it has an opinion (a named starter counts 77%). Brier improves 0.000 on line-ups the fit had not seen |
...
|---|---|---|---|--:|---|---|
| 🟢 | api_activity | every transfer, which is what the ledger replays — one row per deal, so the newest is the last deal and not the last sweep | LaLiga Fantasy API | 144 | 05 Sep 22:45 | fetched 3 minutes ago |
| 🟢 | api_leagues | your cash and the league's id | LaLiga Fantasy API | 155 | 06 Sep 20:09 | fetched 3 minutes ago |
| 🟢 | api_lineup | the eleven you have actually fielded, and the formation the app says you are playing | LaLiga Fantasy API | 1,452 | 06 Sep 20:09 | fetched 3 minutes ago |
| 🟢 | api_market | what is on offer, and the bids on it | LaLiga Fantasy API | 4,662 | 06 Sep 20:09 | fetched 3 minutes ago |
| 🟢 | api_players | names for players nobody owns any more — one row per player, first sighting kept | LaLiga Fantasy API | 109 | 05 Sep 20:29 | fetched 24 hours ago |
| 🟢 | api_standings | the league table — position, points, squad value, and your balance | LaLiga Fantasy API | 775 | 06 Sep 20:09 | fetched 3 minutes ago |
| 🟢 | api_stats | what the app scored each player, broken into what he did — one row per player per week per stat, a correction being a later row rather than an overwrite | LaLiga Fantasy API | 7,063 | 06 Sep 20:09 | fetched 3 minutes ago |
| 🟢 | api_teams | all five squads | LaLiga Fantasy API | 10,945 | 06 Sep 20:09 | fetched 3 minutes ago |
| ⚪ | clubs | the same, for clubs | src/crosswalk.py | 20 | — | rebuilt every run from the tables above |
| 🟢 | elo | team strength, which ranks the fixture term | clubelo.com | 2,740 | 06 Sep 20:09 | fetched 3 minutes ago |
| 🟢 | fixtures | who plays whom next, for the fixture term | analiticafantasy.com | 2,158 | 06 Sep 20:09 | fetched 3 minutes ago |
| 🟢 | lineups | probable XI percentages, both sources | analiticafantasy.com, futbolfantasy.com ×40 | 126,280 | 06 Sep 20:09 | fetched 3 minutes ago |
| 🟢 | market | price, value, position, fitness — every player in the game | futbolfantasy.com | 125,070 | 06 Sep 20:09 | fetched 3 minutes ago |
| 🟢 | matches | fixtures, kickoffs, results | futbolfantasy.com | 60,040 | 06 Sep 20:09 | fetched 3 minutes ago |
...
| 🟢 | points | realised points per jornada, the actuals in every table below | futbolfantasy.com | 1,074 | 05 Sep 22:45 | fetched 3 minutes ago |
| 🟢 | starters | confirmed elevens, which is what P(start) is graded on | futbolfantasy.com | 98,894 | 06 Sep 20:09 | fetched 3 minutes ago |
...
_vs. a trivial guess (everyone scores the sample's own mean, 4.3 pts/match, no player identity at all): ours 3.21 MAE, that guess 3.25 MAE — beats it, adding real information._
...
Still the unfitted default (1.00) — not enough graded pairs yet (h1=32, h3=6, need >=20 each) — keeping 1.00.

### Who to believe about the eleven

| Source | Calls | Mean claim | Hit | Brier |
|---|--:|--:|--:|--:|
| **starts** — 770 confirmed, 5 locked round(s) | | | | |
| analitica | 520 | 84% | 70% | 0.190 |
| futbolfantasy ←read | 1697 | 38% | 37% | 0.085 |
| our forecast | 37 | 72% | 78% | 0.104 |
| analitica — named, no number | 90 | — | 66% | — |

| **starts, same population as our forecast only** — the fair comparison | | | | |
| analitica | 49 | 93% | 76% | 0.235 |
| futbolfantasy ←read | 84 | 68% | 73% | 0.126 |
| our forecast | 37 | 72% | 78% | 0.104 |
...
| our forecast | 316 | 65% | 16% | 0.421 |
| analitica — named, no number | 712 | — | 7% | — |
| futbolfantasy — named, no number | 2 | — | 0% | — |

| Not graded | Calls |
|---|--:|
| within 10 points of 50%, on starts | 347 |
| within 10 points of 50%, on appearances | 1809 |
...
_Our forecast vs. trivial baselines, n=38: ours 0.108, a flat 50% guess 0.250, a constant 72% guess 0.183 (Brier, lower is better) — beats both, adding real information._

## ⚠ Sections missing

These were asked for and not found — a heading was renamed, and `digest.py`'s list needs the new name:

- `sim.md → Act now or wait — the workings`
