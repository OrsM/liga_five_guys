# Liga Five Guys — how the numbers are made — 2026-09-16 04:53 UTC
...
|---|---|---|---|--:|---|---|
| 🟢 | api_activity | every transfer, which is what the ledger replays — one row per deal, so the newest is the last deal and not the last sweep | LaLiga Fantasy API | 202 | 16 Sep 04:50 | fetched 3 minutes ago |
| 🟢 | api_leagues | your cash and the league's id | LaLiga Fantasy API | 194 | 16 Sep 04:50 | fetched 3 minutes ago |
| 🟢 | api_lineup | the eleven you have actually fielded, and the formation the app says you are playing | LaLiga Fantasy API | 1,881 | 16 Sep 04:50 | fetched 3 minutes ago |
| 🟢 | api_market | what is on offer, and the bids on it | LaLiga Fantasy API | 5,929 | 16 Sep 04:50 | fetched 3 minutes ago |
| 🟢 | api_players | names for players nobody owns any more — one row per player, first sighting kept | LaLiga Fantasy API | 130 | 16 Sep 04:50 | fetched 3 minutes ago |
| 🟢 | api_standings | the league table — position, points, squad value, and your balance | LaLiga Fantasy API | 970 | 16 Sep 04:50 | fetched 3 minutes ago |
| 🟢 | api_stats | what the app scored each player, broken into what he did — one row per player per week per stat, a correction being a later row rather than an overwrite | LaLiga Fantasy API | 11,190 | 16 Sep 04:50 | fetched 3 minutes ago |
| 🟢 | api_teams | all five squads | LaLiga Fantasy API | 13,853 | 16 Sep 04:50 | fetched 3 minutes ago |
| ⚪ | clubs | the same, for clubs | src/crosswalk.py | 20 | — | rebuilt every run from the tables above |
| 🟢 | elo | team strength, which ranks the fixture term | clubelo.com | 3,519 | 16 Sep 04:50 | fetched 3 minutes ago |
| 🟢 | fixtures | who plays whom next, for the fixture term | analiticafantasy.com | 2,518 | 16 Sep 04:50 | fetched 3 minutes ago |
| 🟢 | lineups | probable XI percentages, both sources | analiticafantasy.com, futbolfantasy.com ×40 | 150,201 | 16 Sep 04:50 | fetched 3 minutes ago |
| 🟢 | market | price, value, position, fitness — every player in the game | futbolfantasy.com | 150,987 | 16 Sep 04:50 | fetched 3 minutes ago |
| 🟢 | matches | fixtures, kickoffs, results | futbolfantasy.com | 74,860 | 16 Sep 04:50 | fetched 3 minutes ago |
...
| 🟢 | points | realised points per jornada, the actuals in every table below | futbolfantasy.com | 1,700 | 16 Sep 04:50 | fetched 3 minutes ago |
| 🟢 | starters | confirmed elevens, which is what P(start) is graded on | futbolfantasy.com | 177,860 | 16 Sep 04:50 | fetched 3 minutes ago |
...
| Home advantage | +13.1% | yes — fit from 1248 real matches (home 1.51, away 1.16 goals/match, ratio 1.301) |
...
| Season spread, match to match | each round resamples a real per-match score, rescaled to the player's rate | from 1649 observed matches |
...
| Predicted, total | 198 pts |
| Actual, total | 252 pts |
...
_vs. a trivial guess (everyone scores the sample's own mean, 4.4 pts/match, no player identity at all): ours 3.01 MAE, that guess 3.09 MAE — does not clearly beat it yet (90% CI on the gap: -0.81 to +0.71 pts, straddles zero)._
...
|---|--:|--:|--:|--:|
| harder | 28 | 3.5 | 5.0 | -1.4 |
| neutral | 10 | 4.1 | 5.1 | -1.0 |
...
**Fit from real data this run: 1.00** (h3 wasn't more variable than h1 (0.174 vs 0.195, rate_rel-normalised) — no evidence DRIFT_FRAC should move from 1.00).

### Who to believe about the eleven

| Source | Calls | Mean claim | Hit | Brier |
|---|--:|--:|--:|--:|
| **starts** — 1166 confirmed, 52 locked round(s) | | | | |
| analitica | 916 | 82% | 66% | 0.207 |
| futbolfantasy ←read | 2714 | 35% | 35% | 0.072 |
| our forecast | 82 | 67% | 77% | 0.097 |
| analitica — named, no number | 111 | — | 51% | — |

| **starts, same population AND same instances as our forecast only** — the real fair comparison (n=82 claims) | | | | |
| analitica | 51 | 92% | 82% | 0.140 |
| futbolfantasy ←read | 78 | 68% | 78% | 0.077 |
| our forecast | 82 | 67% | 77% | 0.097 |
_vs analitica: 90% CI on the Brier gap -0.129 to +0.018 — no significant difference._
_vs futbolfantasy: 90% CI on the Brier gap -0.024 to +0.047 — no significant difference._
...
| analitica | 2851 | 81% | 19% | 0.591 |
| futbolfantasy ←read | 8545 | 34% | 11% | 0.216 |
| our forecast | 356 | 60% | 18% | 0.384 |
| analitica — named, no number | 503 | — | 12% | — |
| futbolfantasy — named, no number | 3 | — | 0% | — |

| Not graded | Calls |
|---|--:|
| within 10 points of 50%, on starts | 598 |
| jornada 1 — its opener kicked off before this repo saw a kickoff for it, so there is no honest cutoff | all |
| within 10 points of 50%, on appearances | 1732 |
...
_Our forecast vs. trivial baselines, n=88 — a DIFFERENT sample than the tables above (this one joins the rate and start sides on jornada via golden_rows(), no 10-point undecided-band exclusion): ours 0.107, a flat 50% guess 0.250, a constant 66% guess 0.196 (Brier, lower is better) — beats both, adding real information._
