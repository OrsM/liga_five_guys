...
**Gap** is what the XI loses per jornada if he has to play, after re-picking the formation. **€/pt** is value per expected point: the sell shortlist, worst first. **FF** and **AF** are the two probable-XI sources: a benched player both of them expect to start is a different sell from one neither does.
