...
def write_lineup_file(lg, players, path="inputs/lineup.txt"):
    ...
    xi.py falls back to inputs/bench.txt when this file is absent, so the
    old input keeps working.
    """
    from pathlib import Path

    from ffcore.tidy import input_path
    ...
    if path.exists():
        ...
        # First generation: carry over inputs/bench.txt so the switch loses
        # nothing. Everyone not on the old bench starts fielded, which is
        # usually an illegal count — said plainly in the header rather than
        # quietly fixed, because only you know who sits.
        from xi import read_bench
        old_bench = input_path("bench.txt")
        if old_bench.exists():
            benched = {norm(n) for n in
                       read_bench(old_bench.read_text(encoding="utf-8"))}
            prev = {norm(players.get(k, {}).get("name", k)):
                    norm(players.get(k, {}).get("name", k)) not in benched
                    for k in lg.squad(lg.cfg.me)}
            seeded = True
    ...
    write_lines(path, [
        ...
        ] + (["#", "# Seeded from inputs/bench.txt on this first run "
              "— check every mark."] if seeded else []) + [
